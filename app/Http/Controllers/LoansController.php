...
class LoansController extends Controller
{
    public function index(): JsonResponse
    {
        ...
        $dataLoans = Loans::with('user', 'loanItems.book')->get(); // Load relasi juga
    ...
    {
        try {
            $loan = Loans::with('user', 'loanItems.book')->findOrFail($id);
    ...
    public function update(Request $request, $id): JsonResponse
    {
        try {
            ...
            $loan->update($request->only(['user_id', 'loan_date', 'status']));

            return response()->json([
                'message' => $loan->wasChanged()
                    ? 'Data Pinjaman berhasil diperbarui.'
                    : 'Tidak ada perubahan pada data Pinjaman.',
                'data' => $loan
            ], 200);
        } catch (ModelNotFoundException $e) {
            return response()->json(['message' => 'Data Pinjaman tidak ditemukan.'], 404);
